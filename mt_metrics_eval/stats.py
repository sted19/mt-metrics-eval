...
def dcg(gold_scores: ArrayLike, metric_scores: ArrayLike) -> Tuple[float]:
    ...
    return (
        dcg_metric_indices(gold_scores, indices),
        0.0,
    )


def dcg_metric_indices(gold_scores: np.ndarray, indices: np.ndarray) -> float:
...
def dcg_random_tie_break(gold_scores: ArrayLike, metric_scores: ArrayLike) -> float:
    """
    Computes DCG for a single random tie-break:
      1. Sort items by metric_scores (descending).
      2. Identify tie groups (equal metric scores).
      3. Randomly shuffle the items within each tie group.
      4. Concatenate them in descending metric order.
      5. Return the DCG of that single random ranking.
    """
    gold_scores = np.array(gold_scores, dtype=float)
    metric_scores = np.array(metric_scores, dtype=float)

    # 1. Sort by descending metric
    desc_sorted_indices = np.argsort(metric_scores)[::-1]
    sorted_scores = metric_scores[desc_sorted_indices]

    # 2. Identify tie groups
    tie_groups = []
    start_idx = 0
    n = len(sorted_scores)
    while start_idx < n:
        cur_score = sorted_scores[start_idx]
        end_idx = start_idx + 1
        # Find contiguous run of same metric
        while end_idx < n and sorted_scores[end_idx] == cur_score:
            end_idx += 1
        tie_groups.append(desc_sorted_indices[start_idx:end_idx])
        start_idx = end_idx

    # 3. For each tie group, randomly shuffle the items in that group
    final_ranking = []
    for group in tie_groups:
        group_list = list(group)
        random.shuffle(group_list)
        final_ranking.extend(group_list)

    # 4. Compute DCG of this final ranking
    return (dcg_metric_indices(gold_scores, final_ranking), 0.0)


def dcg_permutation_ties(
    gold_scores: ArrayLike, metric_scores: ArrayLike
) -> Tuple[float]:
    """
    ...
    while start_idx < len(sorted_scores):
        ...
        group_end = start_idx
        while (
            group_end < len(sorted_scores) and sorted_scores[group_end] == current_score
        ):
            group_end += 1
    ...
    for gstart, gend in tie_groups:
        group_indices = desc_sorted_indices[
            gstart:gend
        ]  # actual item indices in that tie group
        ...
        group_permutations.append((gstart, perms))

    # Step 5: Compute DCG for each ranking, sum, and average
    dcg = 0.0
    for start_idx, group_perm in group_permutations:
        dcg += average_dcg_across_tie_permutations(start_idx, group_perm, sorted_gold)
...
def average_dcg_across_tie_permutations(
    start_idx, group_metric_indices, gold_scores
) -> float:
    average_dcg = 0
    for indices in group_metric_indices:
        ...
        for i, idx in enumerate(indices):
            rel = gold_scores[idx]
            dcg_sum += rel / np.log2(start_idx + i + 2)
        average_dcg += dcg_sum

    return average_dcg / len(group_metric_indices)


def ndcg(gold_scores: ArrayLike, metric_scores: ArrayLike) -> Tuple[float]:
    """
    ...
    3. Return the ratio: DCG / IDCG --> actually, in our case the smaller the better (in absolute value). Therefore,
        we compute the Normalized DCG as IDCC / DCG, so that the smaller DCG is, the better the score is, 1 being the optimal ratio.

    Returns:
        float: Normalized Discounted Cumulative Gain
        p-value: currently 0.0
    """
    # 1. Compute actual DCG
    actual_dcg = dcg(gold_scores, metric_scores)[0]
    ...
    # Avoid division by zero. Actual DCG can be 0 only if also all ideal_dcg is 0.
    # In that case, it means that all translations have the same score of 0.
    # therefore, any ranking is perfect, and the NDCG should be 1.0

    return (
        ideal_dcg / actual_dcg if ideal_dcg != 0 else 1.0,
        0.0,
    )
