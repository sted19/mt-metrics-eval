...
CORRELATION_FUNCTIONS = {
    ...
    "dcg_random_tie_break": stats.dcg_random_tie_break,
}
...
def _LatexTable(metric_infos, rows, headers, rerank) -> str:
    ...
    for h in headers:
        h = [Esc(x) for x in h]
        ret += (
            h[0] + sep + sep.join([f"\\multicolumn{{2}}{{|l}}{{{x}}}" for x in h[1:]])
        )
        ret += eol + "\n"
    if headers:
        ret += "\\midrule\n"
...
class TaskResults:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.pval == other.pval
            and self.corr_ranks == other.corr_ranks
            and np.array_equal(self.matrix, other.matrix)
            and np.array_equal(self.draws_index, other.draws_index)
            and np.array_equal(self.draws_list, other.draws_list)
        )
    ...
    def range(self) -> tuple[float, float]:
        ...
        if (
            ...
            or corr_fcn == "pce"
            or (
                corr_fcn == "KendallWithTiesOpt"
                and "'variant':'23'" not in self.attr_vals["corr_fcn_args"]
            )
        ):
            return (0, 1)  # accuracy
        else:
    ...
    def Sig(self, m1, m2) -> bool:
        ...
        if isinstance(m1, str):
            m1 = self.metrics.index(m1)
        if isinstance(m2, str):
            m2 = self.metrics.index(m2)
    ...
    def Str(self, probs=False):
        """Return a string representation of metric ranking and significance."""
        return MatrixString(self.corr_ranks, self.matrix, self.pval, probs=probs)
...
class TaskSet:
    ...
    def __init__(self, attr_combs: dict[str, list[Any]] | None = None, **attrs):
    ...
    def Run(
        self, eval_set_dict: dict[tuple[str, str], data.EvalSet] | None = None
    ) -> TaskSetResults:
        """Run all tasks.
...
class TaskSetResults:
    ...
    def AssignWeights(self, attrs: list[str], total_wt: float = 1.0) -> list[float]:
    ...
    def Table(
        ...
        return MetricsTable(
            metrics, columns, headers, fmt, which_metrics, rerank, baselines_metainfo
        )
